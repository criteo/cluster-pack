...
def test_conda_env_from_reqs():
    with tempfile.TemporaryDirectory() as tempdir:
        env_zip_path = conda.create_and_pack_conda_env(reqs=["pycodestyle==2.5.0"])
        assert os.path.isfile(env_zip_path)

        _check_package(tempdir, env_zip_path, "pycodestyle", "2.5.0")
...
def test_conda_env_from_reqs_with_external_repo():
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        _check_package(tempdir, env_zip_path, "torch", "1.10.1+cu113")
...
def test_conda_env_from_reqs_with_external_index():
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        _check_package(tempdir, env_zip_path, "detectron2", "0.6")


def test_conda_env_from_spec():
    spec_file = os.path.join(os.path.dirname(__file__), "resources", "conda.yaml")
    with tempfile.TemporaryDirectory() as tempdir:
        env_zip_path = conda.create_and_pack_conda_env(spec_file=spec_file)
        assert os.path.isfile(env_zip_path)

        _check_package(tempdir, env_zip_path, "botocore", "1.17.12")
